...
window.onload = function(e) {
    ...
    for (let i = 0; i < artwork_videos.length; i++) {
        ...
        if (i === 0) {
            artwork.position.x = -margin;
            artwork.position.z = minZ;
        }

        if (i === 1) {
            artwork.position.x = -margin * 2 - 50;
            artwork.position.z = minZ;
        }
...
var floorMaterial = new THREE.MeshPhongMaterial({ color: 0x000011, side: THREE.DoubleSide, opacity: 0.9, transparent: true });
...
const figure_material = new THREE.MeshPhongMaterial({ color: 0x0022ff, bumpScale: 0.002, map: texture });
...
const skyBoxMaterial = new THREE.MeshPhongMaterial({ color: 0x0022ff, side: THREE.DoubleSide, map: skyBox_texture });
...
// WALL: Front
var texture = new THREE.TextureLoader().load('public/assets/textures/aisle/aisle_texture3.png');
texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
texture.repeat.set(10, 10);
texture.flipY = false;

var wallGeometry = new THREE.PlaneGeometry(map_width, map_width, 10, 10);
var wallMaterial = new THREE.MeshPhongMaterial({ color: 0x0022ff, bumpScale: 0.002, opacity: 0.5, transparent: true, side: THREE.DoubleSide, map: texture });
