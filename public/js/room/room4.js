...
window.onload = function(e) {
    ...
    for (let i = 0; i < artwork_videos.length; i++) {
        ...
        if (i === 3) {
            artwork.position.x = maxX - margin - 50;
            artwork.position.z = 0;
        }

        if (i === 4) {
            artwork.position.x = maxX - margin * 2 - 50 * 2;
            artwork.position.z = 0;
        }
...
var floorMaterial = new THREE.MeshPhongMaterial({ color: 0x001111, side: THREE.DoubleSide, opacity: 0.9, transparent: true });
...
var wallMaterial = new THREE.MeshPhongMaterial({ color: 0x00ffff, bumpScale: 0.002, opacity: 0.5, transparent: true, side: THREE.DoubleSide, map: texture });
