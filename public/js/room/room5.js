...
window.onload = function(e) {
    ...
    for (let i = 0; i < artwork_videos.length; i++) {
        if (i !== 1) {
            ...
            if (i === 6) {
                artwork.position.x = minX + margin + 50;
                artwork.position.z = maxZ / 2;
            }

            if (i === 7) {
                artwork.position.x = minX + margin * 2 + 50 * 2;
                artwork.position.z = maxZ / 2;
            }
...
var floorMaterial = new THREE.MeshPhongMaterial({ color: 0x111111, side: THREE.DoubleSide, opacity: 0.9, transparent: true });
